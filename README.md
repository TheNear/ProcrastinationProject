...
- :white_check_mark:~~При использовании useForm, возвращается объект с values всех инпутов. Так как объект всегда новый, то происходит перерендер всех инпутов сразу.~~
  - :heavy_exclamation_mark: добавлен React.memo на компоненте Input. Сравнивается value и errors.
- :white_check_mark:~~Добавить очистку форм паролей при сабмите форм. Пока часть кода закомментирована в useForm.~~
  - :heavy_exclamation_mark: Очистка происходит в useForm, в useEffect при обновлнении errors.
