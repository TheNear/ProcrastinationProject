...
- Проблема с анимированным демонтированием елемента при клике вне элемента.
  - Элемент убирается при клике на дочерние элементы. Код пока закомментирован в SidePanel.
- Проблем с TextInput, нужен что-бы когда форма в фокусе, кнопка ESC приводила к отмене ввода, но не к закрытию окна. Как оказалась, что работа со stopPropagation() в реакт, это не простая задача.
